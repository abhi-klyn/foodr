...
def lambda_handler(event, context):
    ...
    print('tag', tag)
    res = aggregateResults(rName=rName, foodName=foodName, tag=tag, tag2=tag2, tag3=tag3,
                           calories=calories, exIng1=exIng1, exIng2=exIng2, incIng1=incIng1, incIng2=incIng2)
    print('New', res)
...
def searchFoodByName(foodName):
    rows = runQuery(
        'SELECT foodid  FROM food natural join restaurant where foodname LIKE \'%%{}%%\''.format(foodName))
    foodIdSet = set()
    for row in rows:
        foodIdSet.add(row[0])
    return foodIdSet
...
def searchFoodByRestaurantName(rName):
    rows = runQuery(
        'SELECT foodid  FROM food natural join restaurant where restaurantname LIKE \'%%{}%%\''.format(rName))
    foodIdSet = set()
    for row in rows:
        foodIdSet.add(row[0])
    return foodIdSet
...
def getFoodDetails(foodId):
    result = []
    rows = runQuery(
        'SELECT foodid, restaurantname, foodname, price FROM food natural join restaurant where foodid = {}'.format(foodId))
    for row in rows:
        result.append(row[1])
        result.append(row[2])
        result.append(row[3])
...
def getIngredientList(foodId):
    results = []
    rows = runQuery(
        'SELECT ingredientname FROM food natural join foodingredient natural join ingredient WHERE foodid = {}'.format(foodId))
    for row in rows:
